...
import { Package, ChartLineUp, SignOut, ArrowSquareOut, Clock, List, X } from "@phosphor-icons/react";
...
export function Sidebar({ pharmacyId }: { pharmacyId?: string }) {
  ...
  return (
    <>
      {/* Botão de menu flutuante / header mobile */}
      <div className="md:hidden fixed top-3 right-4 z-50">
        <button
          onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
          className="p-2.5 rounded-xl bg-white shadow-md border border-gray-100 text-gray-700 hover:text-red-600 transition-colors"
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-100 flex flex-col justify-between shrink-0 transition-transform duration-300 ease-in-out ${
      ...
      >
        <div>
          <Link
            href="/"
            onClick={() => setMobileMenuOpen(false)}
            className="h-20 flex items-center px-6 border-b border-gray-50 hover:bg-gray-50 transition-colors"
          >
